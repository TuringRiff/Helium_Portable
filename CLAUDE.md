...
### Release assets are a downstream contract

A Scoop bucket tracks this repository, so the asset layout is not free to change:

- The archive is `Helium_{package_version}.7z`, **with no date**. A Scoop `autoupdate.url` can only interpolate the version, so a date would force the manifest to scrape it back out of the release JSON with a regex — which picks the wrong file whenever a release holds more than one `.7z`.
- Every release also carries `Helium_{package_version}.7z.sha256` in `sha256sum`'s two-column format. That exact format matches Scoop's built-in hash regex, so a manifest needs only `"hash": {"url": "$url.sha256"}` and the bucket's excavator can refresh the digest without downloading ~150 MB.

Forcing a rebuild of an already-published version replaces the archive, so its digest changes while the version does not. Downstream manifests pin the old digest and will fail their hash check until refreshed by hand (`.\bin\checkver.ps1 helium -u -f` in the bucket).
...
- **`scripts/release.py`** — Both halves of the release flow (`check` and `publish`), sharing one `render_tag`, so the two can never disagree about which tag a version belongs to. The tag template is restricted to fields known before the build; allowing `{date}` or `{version}` there would make a rebuild of the same Helium version land on a second tag. `publish` reads the `ARCHIVE_*` / `PACKAGE_VERSION` / `BUILT_VERSION` env vars left by the builder's `build` and `archive` stages, writes the `.sha256` sidecar (cross-checking it against `ARCHIVE_SHA256` so the notes and the sidecar cannot contradict each other), and reuses the builder's `build_context`, `human_size`, `sha256_file` and `archive_name_regex` rather than reimplementing them — hence `PYTHONPATH` on the publish step too.

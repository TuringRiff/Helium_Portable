...
- **`清理Helium安装版注册表.bat` + `scripts/cleanup_helium_registry.ps1`** — Repository-level recovery tool for removing stale installed-Helium Default Apps registrations and resetting only associations that currently point to Helium. It backs up touched keys, supports `--dry-run`, never registers the portable build, and leaves browser files/data alone. Keep the batch file ASCII-only with CRLF line endings because localized `cmd.exe` versions may corrupt UTF-8 batch text. The current builder only copies `start_script`, so this recovery tool is not automatically included in release archives.
